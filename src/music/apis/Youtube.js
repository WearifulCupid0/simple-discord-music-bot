...
module.exports = class YoutubeApi extends Api {
        ...
        return new Promise(async(resolve, reject) => {
            ...
            .then(res => {
                const tracks = res.items.map(track => new Track({
                    ...
                    duration: !track.isLive ? this.getDuration(track.duration) : this.defaultDuration,
                    uri: track.shortUrl,
                    artwork: track.bestThumbnail.url,
                    identifier: track.id,
                    isStream: track.isLive,
                }));
                resolve(new SearchResults({
                    playlistInfo: new Playlist({ title: res.title, author: res.author ? res.author.name : tracks[0].author, uri: res.url, identifier: res.id }),
    ...
    getStream(track) {
        const { uri } = JSON.parse(Buffer.from(track, 'base64').toString('ascii'));
